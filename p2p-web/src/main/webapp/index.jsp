...
<div class="brand-promotion">
    ...
    <div class="mainBox">
        ...
        <div class="jinxin-data">
            <h3>平台用户数</h3>
            <div class="data"><span id="hzxx_ljrs">${totalUser}</span>位</div>
            <div class="line"></div>
            <h3>累计成交额</h3>
...
<div id="productArea">
	...
	<c:forEach items="${noobLoanList}" var="n">
     ...
     <div class="inner">
          <div class="index-exper-top">
               <span class="title"><font style="font-size:28px;">${n.productName}</font></span>
               <p class="text">${n.bidMinLimit}元起投<em>丨
               </em>投资最高限额${n.bidMaxLimit}元<em>丨</em>当日计息</p>
          </div>
          <div class="index-exper-bottom">
               <ul class="list cf">
                   <li class="box1">
                       <p class="text1">${n.rate}<span>%</span></p>
                       <p class="text2">历史年化收益率</p>
                   </li>
                   <li class="box2">
                       <p class="text1">${n.cycle}<span>天</span></p>
                       <p class="text2">投资周期</p>
                   </li>
                   <li class="box3">
                       <p class="text1">${n.leftProductMoney}<span>元</span></p>
                       <p class="text2">剩余可投金额</p>
                   </li>
                   <li class="box4">
					   <a href="${pageContext.request.contextPath}/loan/loanInfo?id=${n.id}" class="go-invest">立即投资</a>
                   </li>
               </ul>
          </div>
     </div>
	...
	<div class="mainBox pro-body clearfix">
		...
		<!-- 1:已满标时class样式追加 pro-full-->
           ...
           <div class="pro-box <c:if test="${e.leftProductMoney==0}">pro-full</c:if>">
			...
			<div class="pro-main">
				...
				<div class="pro-data clearfix">
					<div class="pro-cycle">
						<h3>投资周期</h3>
						<div class="data"><span>${e.cycle}</span>个月</div>
					</div>
					<div class="pro-money">
						<h3>剩余可投金额</h3>
						<div class="data">
							<span>${e.leftProductMoney}</span>元
						</div>
					</div>
				</div>
				<div class="pro-intr"><p>优选计划项目，投资回报周期1个月，起点低，适合短期资金周转、对流动性要求高的投资人。</p></div>
				<div class="pro-btn">
					<a href="${pageContext.request.contextPath}/loan/loanInfo?id=${e.id}" class="btn-1">立即投资</a>
				</div>
			</div>
		</div>
		</c:forEach>
	...
	<div class="mainBox pro-body disperse-pro clearfix">
		...
		<c:forEach items="${commonLoanInfoList}" var="c" varStatus="loop">


		<!-- 1:已满标时追加pro-full-->
            <div class="pro-box <c:if test="${c.leftProductMoney==0}">pro-full</c:if>">
			<div class="pro-top">
				<h3>${c.productName}</h3>
				<p>动力金融网特选小额理财产品</p>
			</div>
			<div class="pro-main">
				<div class="pro-rate">
					<div class="rate">${c.rate}<span>%</span></div>
					<h3>历史年化收益率</h3>
				</div>
				<div class="pro-data clearfix">
					<div class="pro-cycle">
						<h3>投资周期</h3>
						<div class="data"><span>${c.cycle}</span>个月</div>
					</div>
					<div class="pro-money">
						<h3>剩余可投金额</h3>
						<div class="data">
							<span>${c.leftProductMoney}</span>元
						</div>
					</div>
				</div>
				<div class="pro-btn">
					<a href="${pageContext.request.contextPath}/loan/loanInfo?id=${c.id}" class="btn-1">立即投资</a>
				</div>
			</div>
		</div>
